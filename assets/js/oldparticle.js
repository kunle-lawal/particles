...
function checkShapeCollision(dot) {
    for(var i = 0; i < shapes.length; i++) {
        ...
        if(shape.shape === 'square') {
            ...
            // } 
            // if (dot.x < (shape.x + shape.width)) {
            //     if (didCollide) {
            //         console.log("collide x right")
            //         reflect_angle(360);
            //     }
            // } 
            // if (dot.y > shape.y) {
            //     if (didCollide) {
            //         console.log("collide x top")
            //         reflect_angle(360);
            //     }
            // } 
            // if (dot.y < (shape.y + shape.height)) {
            //     if (didCollide) {
            //         console.log("collide x bottom")
            //         reflect_angle(360);
            //     }
            // }
...
function draw_dots() {
    for(var i = 0; i < dots.length; i++) {
        ...
        if(dis_to_center < 1000) {
            ...
            ctx.fillStyle = simulationStarted ? hexToRgbA(dot.color, (dot.life / dot.maxLife)) : colourNameToHexToRgba(dot.color, (dot.life / dot.maxLife));
            ctx.fill();
            ctx.restore();
        }
    }
    ...
    for (let i = 0; i < dots.length; i++) {
        ...
        if (dis_to_center > 1400) {
            if (randNum(1, 1000) < 100) {
                ...
                dot.x = dot.ogX;
                dot.y = dot.ogY;
                // let newX = randNum(0, cw);
                // let newY = randNum(0, ch);
                // dot.vx = (newX - dot.x) / (distToPoint(newX, newY) / 10); //ang;//vPlayer.x;
                // dot.vy = (newY - dot.y) / (distToPoint(newX, newY) / 10); //ang;//vPlayer.y;
                // dot.x = randNum(0, cw);
                // dot.y = randNum(0, ch);
            }
        } else {
...
var addDots = ({ totalParticle, position_x, speed, position_y, velocity_x, velocity_y, gravity_x, gravity_y, particle_size, particle_life, particle_color }) => {
    if(simulationStarted) {
        ...
        for (var i = 0; i < (totalParticle ? (totalParticle > 1000 ? 1000 : totalParticle) : randNum(100, 400)); i++) {
            dots.push({
                x: randNum((cw / 2) - 10, (cw / 2) + 10),
                y: randNum((ch / 2) - 10, (ch / 2) + 10),
                ogX: randNum((cw / 2) - 10, (cw / 2) + 10),
                ogY: randNum((ch / 2) - 10, (ch / 2) + 10),
                r: randNum(0.1, 2),
                vx: randNum(-100, 100) / 7,
                vy: randNum(-100, 100) / 7,
                ogvx: randNum(-100, 100) / 7,
                ogvy: randNum(-100, 100) / 7,
                speed: 0,
                ax: 0,
                ay: 0,
                ogax: 0,
                ogay: 0,
                pos: i,
                life: randNum(100000, 1000000),
                maxLife: randNum(100000, 1000000),
                color: particle_color ? particle_color : 'white',
                stopped: false,
            });
        }
